...
class StudentManagementApp:
    def __init__(self, root):
        ...
        # Make the window 400 pixels wide and 200 pixels tall
        self.root.geometry("400x200")
        # Paint the background bright yellow
        self.root.configure(bg='#FFFF00')  # Set background color to bright yellow

        # Try to put a picture in the window, but if the file isn't there, just skip it
        try:
            # Load the image from a file (you need to put the right file path here)
        ...
        self.btn_add = tk.Button(root, text="Add Student", width=15, command=self.open_student_form, bg='#FFD700')
        self.btn_add.pack(pady=10)

        # Button to search for a student by ID
        self.btn_search = tk.Button(root, text="Search Student", width=15, command=self.search_student, bg='#FFD700')
        self.btn_search.pack(pady=10)

        # Button to show a list of all students
        self.btn_show_all = tk.Button(root, text="Show All Students", width=15, command=self.show_all_students, bg='#FFD700')
        self.btn_show_all.pack(pady=10)

        # Button to close the program
        self.btn_quit = tk.Button(root, text="Quit", width=15, command=self.quit_program, bg='#FFD700')
